...
//hld finish
void move(int howmany, int source, int helper, int target){
    if(howmany==1) {cout<<source<<" "<<target<<"\n"; return;}
    move(howmany-1,source,target,helper);
    cout<<source<<" "<<target<<"\n";
    move(howmany-1,helper,source,target);
}
signed main() {
