...
signed main() {
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            ...
            else if(s[i][j]=='#') parent[i][j]='W';
        }
    }
    parent[start.first][start.second]='S';
    queue<pair<int,int>> q; bool found=0;
    q.push(start);
    while(!q.empty()){
        auto node=q.front(); q.pop();
        if(node==end) {found=1; break;}
        if(node.first>0 && !parent[node.first-1][node.second]) {q.push({node.first-1,node.second}); parent[node.first-1][node.second]='U';}
        if(node.second>0 && !parent[node.first][node.second-1]) {q.push({node.first,node.second-1}); parent[node.first][node.second-1]='L'; }
        if(node.first<n-1 && !parent[node.first+1][node.second]) {q.push({node.first+1,node.second}); parent[node.first+1][node.second]='D';}
        if(node.second<m-1 && !parent[node.first][node.second+1]) {q.push({node.first,node.second+1}); parent[node.first][node.second+1]='R';}
    }
    if(!found) cout<<"NO"<<"\n";
    else{
        cout<<"YES"<<"\n"; string ans;
        while(end!=start){
            ...
            if(parent[end.first][end.second]=='U') end={end.first+1,end.second};
            else if(parent[end.first][end.second]=='L') end={end.first,end.second+1};
            else if(parent[end.first][end.second]=='D') end={end.first-1,end.second};
            else if(parent[end.first][end.second]=='R') end={end.first,end.second-1};
        }
        reverse(all(ans));
        cout<<ans.length()<<"\n"<<ans<<"\n";
    }
