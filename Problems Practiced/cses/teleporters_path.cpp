...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b; cin>>n>>m; vector<vi> adj(n+1);vi indegree(n+1); vi outdegree(n+1);
    for(int i=0; i<m; i++){
        cin>>a>>b;
        adj[a].push_back(b);
        outdegree[a]++; indegree[b]++;
    }
    if(indegree[1]+1!=outdegree[1]) {cout<<"IMPOSSIBLE"; return 0;}
    for(int i=2; i<=n-1; i++){
        if(indegree[i]!=outdegree[i]) {cout<<"IMPOSSIBLE"; return 0;}
    }
    if(outdegree[n]+1!=indegree[n]) {cout<<"IMPOSSIBLE"; return 0;}
    stack<int> st; vi ans;
    st.push(1);
    while(!st.empty()){
        int v=st.top();
        if (adj[v].empty()){
            ...
            int Vnext=adj[v].back();
            adj[v].pop_back();
            st.push(Vnext);
        }
    }
    reverse(all(ans));
    if(ans.size()==m+1) cout<<ans;
