...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b; cin>>n>>m; vector<vi> g(n+1);
    while(m--){
        cin>>a>>b;
        g[a].push_back(b);
    }
    vi visited(n+1),onstack(n+1); vi ans; bool cycle=0; bool unfinished=1;
    auto dfs = [&](auto&& self, int node) -> void {
        visited[node]=1; onstack[node]=1;
        for(auto& child: g[node]){
            if(ans.size()!=0) {if(ans.back()!=ans[0]) ans.push_back(node); return;}
            if(onstack[child]) {ans.push_back(child); ans.push_back(node); return;}
            else if(!visited[child]) self(self,child);
        }
        if(ans.size()!=0) {if(ans.back()!=ans[0]) ans.push_back(node); return;}
        onstack[node]=0;
    };
