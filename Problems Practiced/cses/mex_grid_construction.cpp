...
signed main() {
    ...
    cin.tie(0);
    int n; cin>>n; vector<vi> ans(n,vi(n));
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++){
            vb present(2*n);
            for(int k=0; k<i; k++) if(ans[k][j]<2*n) present[ans[k][j]]=1;
            for(int k=0; k<j; k++) if(ans[i][k]<2*n) present[ans[i][k]]=1;
            for(int k=0; k<2*n; k++) if(!present[k]) {ans[i][j]=k; break;}
        }
    }
