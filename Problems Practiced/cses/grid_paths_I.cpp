...
signed main() {
    ...
    cin.tie(0);
    int n; cin>>n; int mod=1e9+7;
    vector<vector<bool>> inp(n,vector<bool>(n));
    for(auto &l: inp){
        string s; cin>>s;
        for(int i=0;i<n; i++) l[i]=(s[i]=='*');
    }
    vector<vector<int>> ans(n+1,vector<int>(n+1)); ans[1][1]=1-inp[0][0];
    for(int s=3; s<=2*n; s++){
        for(int i=max(1LL,s-n); i<=min(s-1,n); i++){
            if(inp[i-1][s-i-1]) continue;
            ans[i][s-i]=(ans[i-1][s-i]+ans[i][s-i-1])%mod;
        }
    }
    cout<<ans[n][n]<<"\n";
}
