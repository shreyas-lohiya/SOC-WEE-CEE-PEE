...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b,c; cin>>n>>m; vi link(n+1); for(int i=1; i<=n; i++) link[i]=i; vi size(n+1,1);
    ...
    while(m--){
        cin>>a>>b;
        if(!same(a,b,link)) unite(a,b,link,size),count--,maxx=max(maxx,size[find(a,link)]);
        cout<<count<<" "<<maxx<<"\n";
    }
    /*for(auto& edge: edges){
        tie(c,a,b) = edge;
        if(!same(a,b,link)) unite(a,b,link,size),ans+=c,count++;
    }
