...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b,c; cin>>n>>m; vector<tuple<int,int,int>> edges;
    while(m--){
        cin>>a>>b>>c;
        edges.push_back({a,b,c});
    }
    vi distance(n+1,0); bool changed=1; int i=1; vi parent(n+1);
    for (; i<=n && changed; i++) {
        changed=0;
        for (auto e : edges) {
            int a, b, w;
            ...
            if(distance[a]+w<distance[b]) distance[b] = distance[a]+w,changed=1,parent[b]=a;
        }
    }
    if(!changed) cout<<"NO"<<"\n";
    else{
        ...
        for (auto e : edges) {
            ...
            tie(a, b, w) = e;
            if(distance[a]==1e18) continue;
            if(distance[a]+w<distance[b]) end=b;
        }
        vector<bool> onstack(n+1);
        while(!onstack[end]){
            onstack[end]=1; end=parent[end];
        }
        vector<bool> incycle(n+1); vi ans;
        while(!incycle[end]){
            ans.push_back(end); incycle[end]=1; end=parent[end];
        }
        ans.push_back(end);
        reverse(all(ans));
        cout<<"YES"<<"\n"<<ans<<"\n";
