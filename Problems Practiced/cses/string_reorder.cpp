...
signed main() {
    ...
    cin.tie(0);
    string s; cin>>s; int n=s.length(); vi count(26);
    for(int i=0; i<n; i++) count[s[i]-'A']++;
    for(int i=0; i<26; i++){
        if(count[i]>(n+1)/2) {cout<<-1; return 0;}
    }
    int canttake=-1;
    for(int i=0; i<n; i++){
        int m=-1;
        if((n-i)&1) {for(int j=0; j<26; j++) if(count[j]==(n-i+1)/2) {m=j; break;}}
        if(m!=-1) {canttake=m; s[i]='A'+canttake; count[canttake]--; continue;}
        int j=0; while(j==canttake || count[j]==0) j++;
        canttake=j; s[i]='A'+canttake; count[canttake]--;
    }
    cout<<s;
}
