...
signed main() {
    ...
    cin.tie(0);
    int n,m,k,a,b,w; cin>>n>>m>>k; vector<vpi> g(n+1);
    while(m--){
        cin>>a>>b>>w;
        g[a].push_back({b,w});
    }
    vector<vi> distance(n+1); vi processed(n+1); priority_queue<pair<int,int>> q;
    ...
    while (!q.empty()) {
        ...
        for (auto u : g[a]) { 
            ...
            if(processed[b]>=k) continue;
            distance[b].push_back(distance[a][processed[a]]+w); 
            sortv(distance[b]);
            if(distance[b].size()>k) distance[b].pop_back();
            q.push({-distance[b][processed[b]],b});
        }
        processed[a]++;
    }
    cout<<distance[n]<<"\n";
