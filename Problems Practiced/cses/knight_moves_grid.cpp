...
signed main() {
    ...
    cin.tie(0);
    int n; cin>>n; vector<vi> ans(n,vi(n,-1));
    queue<pii> q;
    q.push({0,0});
    ans[0][0]=0;
    while(!q.empty()){
        auto [i,j]=q.front(); q.pop();
        if(i-2>=0 && j-1>=0 && ans[i-2][j-1]==-1) ans[i-2][j-1]=ans[i][j]+1,q.push({i-2,j-1}); 
        if(i-1>=0 && j-2>=0 && ans[i-1][j-2]==-1) ans[i-1][j-2]=ans[i][j]+1,q.push({i-1,j-2});
        if(i-2>=0 && j+1<n && ans[i-2][j+1]==-1) ans[i-2][j+1]=ans[i][j]+1,q.push({i-2,j+1});
        if(i-1>=0 && j+2<n && ans[i-1][j+2]==-1) ans[i-1][j+2]=ans[i][j]+1,q.push({i-1,j+2});
        if(i+2<n && j-1>=0 && ans[i+2][j-1]==-1) ans[i+2][j-1]=ans[i][j]+1,q.push({i+2,j-1});
        if(i+1<n && j-2>=0 && ans[i+1][j-2]==-1) ans[i+1][j-2]=ans[i][j]+1,q.push({i+1,j-2});
        if(i+2<n && j+1<n && ans[i+2][j+1]==-1) ans[i+2][j+1]=ans[i][j]+1,q.push({i+2,j+1});
        if(i+1<n && j+2<n && ans[i+1][j+2]==-1) ans[i+1][j+2]=ans[i][j]+1,q.push({i+1,j+2});
    }
