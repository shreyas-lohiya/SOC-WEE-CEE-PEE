...
signed main() {
    ...
    vi parent(n+1); vi ans;
    queue<int> q; bool found=0;
    q.push(1); parent[1]=1;
    while(!q.empty()){
        if(q.front()==n) {found=1; break;}
        for(auto& c: G[q.front()]){
            if(!parent[c]) parent[c]=q.front(),q.push(c);
        }
        q.pop();
    }
    if(!found) {cout<<"IMPOSSIBLE";return 0;}
    int curr=n;
    while(curr!=1){
        ans.push_back(curr);
        curr=parent[curr];
    }
    ans.push_back(1); reverse(all(ans));
    cout<<ans.size()<<"\n"<<ans<<"\n";
}
