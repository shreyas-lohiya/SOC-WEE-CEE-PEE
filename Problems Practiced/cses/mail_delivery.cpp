...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b; cin>>n>>m; vector<vpi> adj(n+1); vi used(m); vi degree(n+1);
    for(int i=0; i<m; i++){
        cin>>a>>b;
        adj[a].push_back({b,i});
        adj[b].push_back({a,i});
    ...
    for(int i=1; i<=n; i++){
        if(degree[i]%2==1) {cout<<"IMPOSSIBLE"; return 0;}
    }
    stack<int> st; vi ans;
    st.push(1);
    while(!st.empty()){
    ...
    reverse(all(ans));
    if(ans.size()==m+1 && ans.back()==1) cout<<ans;
