...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b,w; cin>>n>>m; vector<vi> g(n+1); vi indegree(n+1);
    while(m--){
        cin>>a>>b;
        g[a].push_back(b);
    ...
    while(!q.empty()){
        ...
        for(auto &c: g[q.front()]){
            indegree[c]--; if(!indegree[c]) q.push(c);
        }
        q.pop();
    }
    vi ans(n+1); ans[1]=1; 
    ...
    for(auto& e: topo){
        if(ans[e]==0) continue;
        for(auto& c: g[e]) {if(ans[e]+1>ans[c]) ans[c]=ans[e]+1,prev[c]=e;}
    }
    if(ans[n]==0) cout<<"IMPOSSIBLE"<<"\n";
    else {
        cout<<ans[n]<<"\n";
        vi order;
        while(n!=0) order.push_back(n),n=prev[n];
        reverse(all(order));
        cout<<order;
    }
}
