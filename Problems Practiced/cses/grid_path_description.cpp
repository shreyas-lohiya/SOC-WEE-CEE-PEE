...
signed main() {
    ...
    string s; cin>>s;
    bool occupied[9][9]={0};
    for(int i=0; i<9; i++) occupied[0][i]=occupied[8][i]=occupied[i][0]=occupied[i][8]=1;
    auto search = [&](auto&& self, int i ,int j, int moveno) -> int {
        if(moveno==48 && i==7 && j==1) return 1;
        if(i==7 && j==1) return 0;
        int ans=0;
        if(s[moveno]=='?'){
            if(occupied[i][j-1] && occupied[i][j+1] && !occupied[i-1][j] && !occupied[i+1][j]) return 0;
            if(!occupied[i][j-1] && !occupied[i][j+1] && occupied[i-1][j] && occupied[i+1][j]) return 0;
            if(!occupied[i][j-1]){occupied[i][j-1]=1; ans+=self(self,i,j-1,moveno+1); occupied[i][j-1]=0;}
            if(!occupied[i][j+1]){occupied[i][j+1]=1; ans+=self(self,i,j+1,moveno+1); occupied[i][j+1]=0;}
            if(!occupied[i-1][j]){occupied[i-1][j]=1; ans+=self(self,i-1,j,moveno+1); occupied[i-1][j]=0;}
            if(!occupied[i+1][j]){occupied[i+1][j]=1; ans+=self(self,i+1,j,moveno+1); occupied[i+1][j]=0;}
        }
        else if(s[moveno]=='U' && !occupied[i-1][j]){occupied[i-1][j]=1; ans+=self(self,i-1,j,moveno+1); occupied[i-1][j]=0;}
        else if(s[moveno]=='D' && !occupied[i+1][j]){occupied[i+1][j]=1; ans+=self(self,i+1,j,moveno+1); occupied[i+1][j]=0;}
        else if(s[moveno]=='L' && !occupied[i][j-1]){occupied[i][j-1]=1; ans+=self(self,i,j-1,moveno+1); occupied[i][j-1]=0;}
        else if(s[moveno]=='R' && !occupied[i][j+1]){occupied[i][j+1]=1; ans+=self(self,i,j+1,moveno+1); occupied[i][j+1]=0;}
        return ans;
    };
    occupied[1][1]=1;
    cout<<search(search,1,1,0);
}
