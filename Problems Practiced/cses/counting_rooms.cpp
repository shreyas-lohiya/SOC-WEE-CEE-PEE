...
signed main() {
    ...
    int ans=0;
    auto dfs = [&](auto&& self, pair<int,int> node, pair<int,int> parent) -> void {
        if(visited[node.first][node.second]) return;
        visited[node.first][node.second]=1;
        if(node.first>0) self(self,{node.first-1,node.second},node);
        if(node.second>0) self(self,{node.first,node.second-1},node);
        if(node.first<n-1) self(self,{node.first+1,node.second},node);
        if(node.second<m-1) self(self,{node.first,node.second+1},node);
    };
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if(!visited[i][j]) {ans++; dfs(dfs,{i,j},{-1,-1});};
        }
    }
    cout<<ans<<"\n";
}
