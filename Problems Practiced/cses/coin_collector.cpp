...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b; cin>>n>>m; vi k(n+1); for(int i=1; i<=n; i++) cin>>k[i]; vector<vi> g(n+1),gt(n+1); vi ans(1); int prev=0;
    while(m--){
        cin>>a>>b;
        g[a].push_back(b);
        gt[b].push_back(a);
    }
    vi ord; vb v(n+1); vi comp(n+1); int currcomp=0;
    ...
    auto dfs2 = [&](auto&& self, int node) -> void {
        comp[node]=currcomp; ans[currcomp]+=k[node];
        for(auto& child: gt[node]){
            if(!comp[child]) self(self,child);
            else if(comp[child]!=currcomp) prev=max(prev,ans[comp[child]]);
        }
    };
    for(int i=1; i<=n; i++){
        if(!v[i]) dfs1(dfs1,i);
    ...
    for(int i=n-1; i>=0; i--){
        if(!comp[ord[i]]){
            currcomp++; prev=0; ans.push_back(0);
            dfs2(dfs2,ord[i]);
            ans[currcomp]+=prev;
            best=max(best,ans[currcomp]);
        }
    }
