...
signed main() {
    ...
    cin.tie(0);
    int n,m,a,b,mod=1e9+7; cin>>n>>m; int p=1<<(n-1); vector<vi> adj(n+1,vi(n+1)); vector<vi> adjback(n+1);
    while(m--){
        cin>>a>>b;
        if(!adj[a][b]) adjback[b].push_back(a);
        adj[a][b]++;
    }
    vector<vi> ans(p,vi(n)); //ans[s][i] would represent ans(subset=s,end=i) subset=00010 represent 54321 2 present only
    ans[1][1]=1;
    for(int s=0; s<p; s++){
        for(int i=2; i<n; i++){
            if(!(s & 1)) continue;
            if(!(s & (1<<(i-1)))) continue;
            int s_x = s ^ (1<<(i-1));
            for(auto& j: adjback[i]){
                if(j==n) continue;
                ans[s][i]+=adj[j][i]*ans[s_x][j]; ans[s][i]%=mod;
            }
        }
    }
    int i=n; int s_x=p-1; int finalans=0;
    for(auto& j: adjback[i]){
        finalans+=adj[j][i]*ans[s_x][j]; finalans%=mod;
    }
    cout<<finalans;
}
