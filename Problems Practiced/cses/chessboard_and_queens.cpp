...
//hld finish
void prin(string prefix, vi& count){
    bool p=1;
    for(int i=0; i<26; i++){
        if(count[i]>0) {
            p=0;
...
signed main() {
    ...
    cin.tie(0);
    vector<string> s(8); cin>>s;
    int ans=0;
    vb col(8),diag1(15),diag2(15);
    auto calc = [&](auto&& self, int layer) -> void {
        if(layer==8) {ans++; return;}
        for(int i=0; i<8; i++){
            if(s[layer][i]=='*' || col[i] || diag1[layer+i] || diag2[7+layer-i]) continue;
            col[i]=1;
            diag1[layer+i]=1;
            diag2[7+layer-i]=1;
            self(self,layer+1);
